...
import { User, LauncherSettings, Version, VersionType, VersionManifest, LaunchConfig, Instance } from '../types';
...
const Home: React.FC<HomeProps> = ({ user, settings, selectedInstance, onResetInstance, onToggleLanguage }) => {
    const [versions, setVersions] = useState<Version[]>([]);
    const [selectedVersion, setSelectedVersion] = useState<string>('');
    const [searchTerm, setSearchTerm] = useState('');
    const [isLaunching, setIsLaunching] = useState(false);
    const [progress, setProgress] = useState(0);
    const [status, setStatus] = useState(t('standby', (settings.language as 'en' | 'ru') || 'en'));
    ...
    useEffect(() => {
        const fetchManifest = async () => {
            try {
                ...
                setStatus(t('readyToLaunch', lang));
            } catch (error) {
                console.error('Manifest fetch error:', error);
                setStatus(t('connectionError', lang));
                setLoadingManifest(false);
            }
        };
        fetchManifest();
    ...
    const handleLaunch = async () => {
        ...
        if (!versionData.url) {
            alert(`Error: No download URL for version ${selectedVersion}`);
        ...
        try {
            setStatus('Checking Version Files');
            setProgress(5);
            ...
            if (window.electron) {
                ...
                window.electron.onDownloadProgress?.((data) => {
                    console.log('[UI] Download progress:', data);
                    addLog(data.status);
                    setProgress(15 + (data.progress * 0.35)); // От 15% до 50%
                    setStatus(data.status);
                });
                
                // Ждем завершения скачивания используя Promise с таймаутом
                try {
                    console.log('[UI] Starting download for version:', selectedVersion);
                    console.log('[UI] Download URL:', versionData.url);
                    
                    // @ts-ignore
                    const downloadPromise = window.electron.downloadVersion(versionData.url);
                    
                    // Таймаут на 5 минут
                    const timeoutPromise = new Promise((_, reject) => 
                        setTimeout(() => reject(new Error('Download timeout (5 minutes)')), 5 * 60 * 1000)
                    );
                    
                    const result = await Promise.race([downloadPromise, timeoutPromise]);
                    console.log('[UI] Download result:', result);
                    addLog('Version files downloaded successfully');
                } catch (downloadError) {
                    console.error('[UI] Download error:', downloadError);
                    addLog(`Download error: ${downloadError instanceof Error ? downloadError.message : String(downloadError)}`);
                    throw downloadError;
                }
            }
            ...
            // @ts-ignore
            if (window.electron) {
                // Convert Base64-encoded UUID to proper UUID format
                let uuidForMinecraft = '00000000-0000-0000-0000-000000000000';
                if (user?.uuid) {
                    // If uuid looks like base64 (Keephit -> Tm9taW5vcg==), convert to proper UUID
                    const uuid = user.uuid;
                    if (uuid.includes('-')) {
                        // Already in UUID format (cloud-xxxx)
                        uuidForMinecraft = uuid.padEnd(36, '0').substring(0, 36);
                        if (!uuidForMinecraft.includes('-')) {
                            // Fallback: create UUID from base64
                            const hex = Buffer.from(uuid, 'utf8').toString('hex');
                            uuidForMinecraft = `${hex.substring(0, 8)}-${hex.substring(8, 12)}-${hex.substring(12, 16)}-${hex.substring(16, 20)}-${hex.substring(20, 32)}`;
                        }
                    } else {
                        // Create proper UUID from any string
                        const hex = Buffer.from(uuid, 'utf8').toString('hex').padEnd(32, '0');
                        uuidForMinecraft = `${hex.substring(0, 8)}-${hex.substring(8, 12)}-${hex.substring(12, 16)}-${hex.substring(16, 20)}-${hex.substring(20, 32)}`;
                    }
                }
                ...
                window.electron.launchGame({ 
                    ...
                    uuid: uuidForMinecraft,
                    accessToken: user?.accessToken || '0',
                    userType: user?.type || 'offline',
                    instanceName: selectedInstance?.name || undefined
                });
                
                // Слушаем ответ о запуске игры
                // @ts-ignore
                window.electron.onLaunchResponse?.((response) => {
                    if (!response.success) {
            ...
            // Update Discord RPC to show playing status
            if (settings.rpcEnabled && selectedInstance) {
                console.log('Updating Discord RPC to playing status');
                discordRPC.updatePlayingPresence(
    ...
    return (
        <div className="h-full flex flex-col justify-between animate-fadeIn">
            <div className="flex-1 flex flex-col justify-center items-center text-center animate-float relative">
                ...
                {isLaunching && (
                    <div className="w-full max-w-xl mt-4 animate-fadeIn">
                        <div className={`glass p-6 rounded-[2rem] text-left font-mono text-[10px] h-48 overflow-y-auto flex flex-col justify-end gap-1.5 transition-all duration-500 ${isLight ? 'text-slate-600 border-slate-300/50' : 'text-purple-200/40 border-purple-500/20 shadow-inner shadow-purple-900/10'}`}>
                            {logs.map((log, i) => (
            ...
            <div className="w-full flex flex-col items-center mb-8 px-6 gap-4">
                    ...
                    <div className="flex items-center gap-12">
                        ...
                        <div className="relative flex-[1.6]">
                            <div className="flex justify-between items-center mb-4 px-3">
                                <label className={`text-[10px] font-black uppercase tracking-[0.2em] ${isLight ? 'text-slate-400' : 'text-white/20'}`}>
                                    {selectedInstance ? `${t('loadedEngine', lang)}: ${selectedInstance.name}` : t('coreReleaseModule', lang)}
                                </label>
                                {selectedInstance && (
                                    <button 
                                ...
                                <>
                                    ...
                                    >
                                        <div className="flex items-center gap-4">
                                            ...
                                            <span className={`font-black tracking-tight text-sm ${isLight ? 'text-slate-700' : 'text-white'}`}>{loadingManifest ? t('syncingManifest', lang) : `${t('vanillaVersion', lang)} ${selectedVersion}`}</span>
                                        </div>
                                        ...
                                        <div className={`absolute bottom-full mb-8 left-0 w-full glass rounded-[3rem] border shadow-[0_30px_60px_rgba(0,0,0,0.5)] overflow-hidden z-[100] animate-fadeInUp ${isLight ? 'border-slate-300/40 bg-white/95' : 'border-white/10 bg-[#0c0c0e]/98'}`}>
                                            <div className={`p-6 border-b ${isLight ? 'border-slate-200' : 'border-white/5'}`}>
                                                <input 
                                                    autoFocus
                                                    type="text" 
                                                    placeholder={t('searchVersionDatabase', lang)} 
                                            ...
                                            <div className="max-h-80 overflow-y-auto py-4 custom-scrollbar">
                                                {filteredVersions.map(v => (
                                                    ...
                                                    >
                                                        ...
                                                        <span className={`text-[10px] font-black uppercase tracking-[0.2em] ${isLight ? 'text-slate-400' : 'text-white/10'}`}>{v.type} {t('moduleType', lang)}</span>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
                        ...
                        <div className="flex-[1.8] flex flex-col justify-center">
                            <div className="flex justify-between items-end mb-5">
                                <div className="flex flex-col">
                                    <span className={`text-[10px] font-black uppercase tracking-[0.2em] mb-1.5 ${isLight ? 'text-slate-400' : 'text-white/10'}`}>{t('engineTelemetry', lang)}</span>
                        ...
                        {/* Launch Trigger */}
                        <button
                            disabled={isLaunching || loadingManifest}
                        ...
                        >
                            <span className="relative z-10">{isLaunching ? t('running', lang) : t('play', lang)}</span>
                            {!isLaunching && !loadingManifest && (
